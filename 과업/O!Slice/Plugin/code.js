...
async function countAreaViolations(frame) {
    let count = 0;
    // 1. 이름이 computeFrameName 결과와 다름
    const expected = await computeFrameName(frame);
    if (expected !== null && expected !== frame.name)
        count++;
    ...
    if (frame.parent && frame.parent.type === 'FRAME' && frame.parent.layoutMode === 'VERTICAL') {
...
async function detectEdgeCases(node, issues) {
    ...
    if (node.type === 'FRAME') {
        ...
        if (frame.parent &&
            frame.parent.type === 'FRAME' &&
            frame.parent.layoutMode === 'VERTICAL') {
            const sizing = frame.layoutSizingHorizontal;
            if (sizing !== undefined && sizing !== 'FILL') {
                issues.push({
