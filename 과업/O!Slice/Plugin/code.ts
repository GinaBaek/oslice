...
async function computeFrameName(frame: FrameNode): Promise<string | null> {
  ...
  if (['Body', 'Status Bar', 'Top Bar', 'CTA Bar', 'List', 'Row'].includes(frame.name)) return null;
...
async function applyAreaGrouping(frame: FrameNode): Promise<RevertOp[]> {
  const ops: RevertOp[] = [];
  const instances = [...frame.children].filter(c => c.type === 'INSTANCE') as InstanceNode[];
  if (instances.length === 0) return ops;

  const bodySnap = snapshotFrame(frame);
  const bodyPaddingTop = frame.paddingTop;

  if (frame.layoutMode === 'NONE') applyAutoLayout(frame);
  ...
  if (node.type === 'FRAME') {
    ...
    if (
      ...
      if (isListLike) {
        ...
        const target = newName || ((await getCompName(children[0] as InstanceNode)) + ' Area');
        issues.push({
          type: 'redundant-wrapper',
    ...
    // 이름 검사
    if (frame.parent && (frame.parent as any).type !== 'PAGE' && !issues.find(i => i.nodeId === frame.id)) {
...
async function fixExcessTopPadding(nodeId: string): Promise<RevertOp[]> {
  ...
  if (prev.type !== 'FRAME') throw new Error('이전 노드가 프레임이 아니에요.');
  const prevFrame = prev as FrameNode;
  const ops: RevertOp[] = [
...
async function fixRedundantWrapper(nodeId: string): Promise<RevertOp[]> {
  ...
  if (frameChildren.length === 1 && frameChildren[0].type === 'INSTANCE') {
    ...
    const newName = await computeFrameName(frame);
    const target = newName || ((await getCompName(frameChildren[0] as InstanceNode)) + ' Area');
    frame.name = target;
